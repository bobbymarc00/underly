...
export function sha256Bytes(bytes: Uint8Array): string {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + upperSigma1 + choice + SHA256_ROUND[index] + words[index]) >>> 0;
...
export function sha256Calldata(value: string): string {
  if (!/^0x(?:[a-fA-F0-9]{2})+$/.test(value)) {
    throw new Error("transaction calldata must be nonempty, even-length hexadecimal bytes");
  }
